...
function Worldmap() {
    useEffect(() => {
        ...
        .defer(d3.csv, "http://127.0.0.1:5000/d3_data.csv", function(d) { data.set(d.country_code, +d.country_score);})
        .await(ready);
        ...
        let mouseClick = function(d) {
            ...
            d3.json("http://127.0.0.1:5000/orgs.json", function(error, orgs) {
                ...
                .on("click", function(d) {
                    ...
                    var svg3_exist = document.getElementById("auth_bar_chart");
                    if (svg3_exist) {
                        d3.select("#auth_bar_chart").remove();
                    }
                    d3.select(this)
                    var width3 = 300,
                    height3 = 150;
                    ...
                    .style("font-size", "12px")
                    .style("text-decoration", "underline")
                    .text("Top Author based in selected institution");
                    for (var org in country_obj){
                        // check if org has key == d.org
                        if (org === d.org){
                            var org_obj = country_obj[org]
                        }
                    }
                    // find the author info
                    var auth_info = []
                    var auth_score = []
                    for (var auth in org_obj){
                        // check if auth is a dict
                        if (typeof org_obj[auth] === "object"){
                            // get the auth key
                            auth_info.push(auth)
                            console.log(org_obj[auth]["author_score"])
                            auth_score.push(org_obj[auth]["author_score"])
                        }
                    }
                    var auth_data = auth_info.map((el, i) => ({auth: el, score: auth_score[i]}))
                    auth_data.sort(function(b,a){
                        return a.score - b.score
                    })
                    ...
                    // create x_axis for the bar chart
                    var x_axis_2 = d3.scaleBand()
                    .range([0, width3])
                    .domain(auth_data.map(function(d) { return d.auth; }))
                    .padding(0.2);

                    //add x axis to svg
                    svg3.append("g")
                    .attr("transform", "translate(0," + height3 + ")")
                    .call(d3.axisBottom(x_axis_2))
                    .selectAll("text")
                    .attr("transform", "translate(-10,0)rotate(-45)")
                    .style("text-anchor", "end");

                    // create y_axis for the bar chart
                    var y_axis_2 = d3.scaleLinear()
                    .range([height3, 0])
                    .domain([0, d3.max(auth_data, function(d) { return d.score; })]);

                    // add y axis to svg
                    svg3.append("g")
                    .call(d3.axisLeft(y_axis_2));
                    ...
                    .enter()
                    .append("rect")
                    .attr("x", function(d) { return x_axis_2(d.auth); })
                    .attr("y", function(d) { return y_axis_2(d.score); })
                    .attr("width", x_axis_2.bandwidth())
                    .attr("height", function(d) { return height3 - y_axis_2(d.score); })
                    .attr("fill", function(d) { return colorScale(d.score); })
                    .on("click", function(d) {
        ...
        svg.append("g")
            ...
            .on("mousemove", function(){return tooltip_country.style("top", (d3.event.pageY ) + "px").style("left", (d3.event.pageX-100) + "px")})
