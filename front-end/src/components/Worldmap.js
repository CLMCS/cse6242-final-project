...
function Worldmap(props) {
    // props.fieldName = 'materialsEngineering'
    const url_country = props.fieldName === 'unselected'?
    `http://localhost:5000/init_country_score`
    :`http://localhost:5000/api/map/country/${props.fieldName}`
    ...
    useEffect(() => {
        ...
        .defer(d3.csv, `${url_country}`, function(d) { data.set(d.country_code, +d.country_score);})
        .await(ready);
        ...
        let mouseClick = function(d) {
            ...
            d3.json(`http://127.0.0.1:5000/api/map/org/${props.fieldName}`, function(error, orgs) {
                ...
                for (var org in country_obj){
                    ...
                    if (typeof country_obj[org] === "object"){
                        ...
                        org_id.push(country_obj[org]["org_id"]) 
                    }
                }
                var org_data = org_info.map((el, i) => ({org: el, score: org_score[i], id: org_id[i]})) 
                ...
                .on("click", function(d) {
                        // console.log(d.org + ": " + country+  "score: "+ d.score + "id: " + d.id)
                        window.open(`http://localhost:3000/datatable/author-rank/by-org/${props.fieldName}/${d.id}/100`);
